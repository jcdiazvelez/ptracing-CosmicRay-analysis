...
def get_reweighed_particles(particles, num_bins, nside, phys_index, model_index):
    ...
    for item in particles:
        ...
        for i in range(num_bins):
            if p >= bin_sizes[i]:
                ...
                break
        final_maps[p_bin][final_pixel] += \
            weight_powerlaw(p, bin_sizes[0], bin_sizes[-1], phys_index, model_index) * \
            observational_weight(p)
    ...
    for item in particles:
        ...
        reweighed_final[final_pixel].append([p, momentum_weight * dipole_weight * obs_weight / direction_weight, t])
